...
class Booking(webdriver.Chrome):
    def __init__(self, driver_path=r"C:\SeleniumDrivers", teardown=False):
        ...
        #use implicitly_wait for making each execution to wait 15min before execution
        self.implicitly_wait(15)
        #it use to make the window to be in full form
        self.maximize_window()

    #this is use to exit the bot after the automation finishes
    ...
    def select_dates(self, check_in_date, check_out_date):
        check_in_element=self.find_element_by_css_selector(
            f'td[data-data="{check_in_date}"]'
        )
        check_in_element.click()

        check_out_element=self.find_element_by_css_selector(
            f'td[data-data="{check_out_date}"]'
        )
        check_out_element()

    def select_adults(self, count=1):
        selection_element = self.find_element_by_id('xp__guest__toggle')
